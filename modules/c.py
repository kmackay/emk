...
class _ClangStaticCompiler(object):
    """
    Compiler class for using clang static analyzer to compile C/C++ respectively.

    Properties (defaults set based on the path prefix passed to the constructor):
      c_path   -- The path of the C compiler (eg "ccc-analyzer").
      cxx_path -- The path of the C++ compiler (eg "c++-analyzer").
    """
    def __init__(self, path_prefix=""):
        """
        Create a new StaticCompiler instance.

        Arguments:
          path_prefix -- The prefix to use for the clang executables.
        """
        self.c_path = path_prefix + "ccc-analyzer"
        self.cxx_path = path_prefix + "c++-analyzer"

    def load_extra_dependencies(self, path):
        """
        Load extra dependencies for the given object file path. The extra dependencies
        could be loaded from a generated dependency file for that path, or loaded from
        the emk.scope_cache(path) (or some other mechanism).
    ...
    def depfile_args(self, dep_file):
        """
        Returns a list of arguments to write secondary dependencies to the given
        dep_file path.
        """
        return ["-Wp,-MMD,%s" % (dep_file)]
    ...
    def compile_c(self, source, dest, includes, defines, flags):
        ...
        Arguments:
          ...
          dest     -- The output object file path.
          includes -- A list of extra include directories.
          defines  -- A dict of <name>: <value> entries to be used as defines.
          flags    -- A list of additional flags. This list may contain tuples.
        """
        self.compile(self.c_path, source, dest, includes, defines, flags)
